...
export function VerticalShort({ short }: { short: Short }) {
  ...
  const timeOf = useCallback(() => {
    ...
    return silentElapsed.current + (performance.now() - silentStart.current) / 1000;
  }, [hasAudio]);

  const stop = useCallback(() => {
    if (!hasAudio) silentElapsed.current += (performance.now() - silentStart.current) / 1000;
    setPlaying(false);
